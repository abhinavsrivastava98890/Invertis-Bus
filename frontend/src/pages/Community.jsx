...
const Community = () => {
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/grievances');
  ...
  const handlePostComplaint = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/grievance', payload);
          ...
          <div key={comp._id} className="glass animate-slide-up" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f0f0f0', paddingTop: '1rem', marginTop: '0.5rem' }}>
              <button style={{ 
                ...
                color: 'var(--text-light)', fontWeight: '600', cursor: 'pointer' 
              }}>
                <ThumbsUp size={18} /> {comp.upvotes} Agree
              </button>
            ...
            <form onSubmit={handlePostComplaint} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button type="button" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0.75rem', borderRadius: '8px', border: '1px solid #e0e0e0', backgroundColor: 'transparent', cursor: 'pointer', color: 'var(--text-dark)' }}>
